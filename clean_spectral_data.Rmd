...
interp_wl <- all_spectra %>%
  ...
  pblapply(function(df) {
    ...
    #VS-NOTE: debug
    print(as.character(unique(df$spectraID))) 
    #if(df$spectraID %in% c("NEON.PLA.D11.CLBJ.00485_2017")){
    #  print(df$wavelength_nm)
    #  print(df$reflectance)
    #}
...
```{r}
# VS-NOTE debug interpolation issue

trouble_spectrum <- all_spectra %>% 
    dplyr::filter(spectraID %in% c("NEON.PLA.D11.CLBJ.00485_2017")) 

message("reflectance value summary:")
summary(trouble_spectrum$reflectance)

message("wavelength value summary")
summary(trouble_spectrum$wavelength_nm)

# plot spectrum
trouble_spectrum %>% 
    ggplot(aes(x = wavelength_nm, y = reflectance)) +
    geom_line() + 
    labs(title = "NEON.PLA.D11.CLBJ.00485_2017") + 
    theme_bw()

# attempt to interpolate manually
wl_orig <- trouble_spectrum %>% 
...
# This works, but the function below still gets stuck on this spectraID
# https://stackoverflow.com/questions/44386686/interpolation-function-approx-in-r-gives-error-need-at-least-two-non-na-valu
# ^ says the approx function needs two complete (x,y) pairs, can't have duplicated x values

# What if I use my less efficient interpolation code from earlier - 
...
Now, each spectrum (designated by a unique spectraID value) has the same 426 wavelength values. 
...
```{r}
# list of all unique uid.x values
unique_uids <- unique(filtered_spectra$uid.x)

# Random sample indices
set.seed(61)
# train 0.6, test 0.2, eval 0.2
train_index <- sample(1:length(unique_uids), 
                      0.6 * length(unique_uids))
test_eval_indices <- setdiff(1:length(unique_uids), train_index)
...
# create a lookup table with columns for uid.x and set name
set_lut <- data.frame(uid.x = unique_uids,
                      set = NA)
set_lut[train_index, 2] <- "train"
set_lut[test_index, 2] <- "test"
set_lut[eval_index, 2] <- "eval"

# see how many uid.x values are labeled as train, test, eval
set_lut %>%
  group_by(set) %>% 
  tally() %>% 
...
write.csv(x = filtered_spectra,
          file = here::here("data", "cleaned spectra.csv"),
          row.names = FALSE)
```
