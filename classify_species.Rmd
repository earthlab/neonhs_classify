...
```{r formatted_conf_matrix, fig.height=7, fig.width=7}
...
confMatrix
```
Creating this confusion matrix to illustrate classification results for the TRAINING SET using ggplot: 

```{r randomForest_train_confMatrix, fig.width=8, fig.height=8}
...
```{r randomForest_valid_confMatrix, fig.width=8, fig.height=8}
...
print(paste("Overall Accuracy of validation set species predictions:", round(OA_randomForest_valid, 3)))
```
