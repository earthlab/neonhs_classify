...
ggplot(imp_top30_mda, aes(x=reorder(varnames, MeanDecreaseAccuracy), 
  ...
  theme_minimal()
```
Perhaps variable importance would be more useful to illustrate as a function of wavelength...
...
print(rf_classifier_genus)
```

Predict genus for the validation set, and create confusion matrix and variable importance plots.

```{r randomForest_valid_confMatrix_genus, fig.width=6, fig.height=6, message=FALSE}
# predict species for validation set
...
```{r randomForest_varImpPlot_ggplot_genus}
...
```{r varImp_MDA_genus, fig.height=4, fig.width=8}
...
```{r varImp_Gini_genus, fig.height=4, fig.width=8}
ggplot(imp_genus, aes(x = wavelength_nm,  
                y = MeanDecreaseGini)) + 
  # add bars where each height is equal to the variable importance metric
  geom_col(aes(fill = MeanDecreaseGini), color = NA) +
...
```{r caret, include=FALSE, eval = FALSE}
