...
```{r randomForest_test, fig.width=12, fig.height=12}
...
# confusion matrix
rf_test_results <- data.frame(obs = test_data$genusSpecies,
                              pred = as.vector(p))
levels(rf_test_results$pred) <- levels(rf_test_results$obs)

# Use the yardstick library to calculate confusion matrix
...
autoplot(cm, type = "heatmap") + 
  scale_fill_gradient(low="#FFFFFF",high = "#2E86C1") + 
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

# VS-NOTE how to adjust the color scale to show greater contrast with low values? 
```

### Variable importance 

```{r randomForest_varImpPlot_default, fig.height=10}
