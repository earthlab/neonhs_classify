...
spectra_locations <- all_spectra %>% 
  select(latitude = adjDecimalLatitude, longitude = adjDecimalLongitude, domainNumber, siteCode) 

# convert the CRS for mapping 
states_abe <- st_transform(x = states, crs = 2163)
world_abe <- st_transform(x = world, crs = 2163)
...
main_map +
  ...
  labs(fill = "NEON Domain")
```
Add continental boundaries for context? Fix background for Alaska inset
...
main_map +
  ...
  labs(fill = "NEON Domain")
```
Add a north arrow and fix the inset size
...
colourCount = length(unique(domains_filtered$DomainID))
#getPalette = colorRampPalette(brewer.pal(colourCount, "Spectral"))
getPalette = viridis(colourCount) # use the viridis palette 
colors_domains <- data.frame(names = sort(unique(domains_filtered$DomainName)),
                        #values = getPalette(colourCount)) #colorRampPalette
                        values = getPalette) # viridis
...
# to do? 
# remove D10 since there are no points located within it? 
# change the size of each point to show how many spectra per site???? 
```
