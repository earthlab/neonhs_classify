...
st_intersection takes too long using all points... somehow get a shorter list of which domains have data? 
```{r}
# transform CRS of spectra locations
spectra_points <- st_as_sf(x = spectra_locations, 
                           coords = c("longitude", "latitude"), 
                           crs = 2163)
neon_domains_aee <- st_transform(x = neon_domains, crs =2163)

# filter NEON Domains based on where we have veg samples 
neon_domains_subset <- st_intersection(x = neon_domains_aee,
                                       y = spectra_points)
```
