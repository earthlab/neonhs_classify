...
# add a new column with domain name code 
all_spectra <- all_spectra %>% 
  tidyr::separate(col = namedLocation, into = "domainCode", sep = "_", extra = "drop")
```
...
domains_filtered <- neon_domains %>%
  ...
  filter( id %in% unlist( st_intersects(x = neon_domains, y = spectra_points) ) ) 

mapview(domains_filtered, zcol = "DomainName") + mapview(spectra_points)


neon_domains_aee <- st_transform(x = neon_domains, crs =2163)

# filter NEON Domains based on where we have veg samples 
neon_domains_aee %>%
  mutate( id = row_number() ) %>% 
  #filter out domains that do not have any intersections with the points
  filter( id %in% unlist( st_intersection(x = neon_domains_aee, y = spectra_points) ) ) %>%
  #plot
  mapview::mapview()   
...
main_map <- ggplot() + 
  ...
  geom_sf(data = us_states_2163, fill="grey", color="gray60", alpha=0.3) + 
   # add NEON domains 
  geom_sf(data = neon_domains_subset, aes(fill = factor(DomainName)), 
          alpha = 0.7, color = "gray40") + 
  # get rid of legend
  theme(legend.position = "none") + 
  # set the map extent to make room for the inset 
